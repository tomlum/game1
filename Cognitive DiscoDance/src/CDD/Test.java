...
public class Test {
    ...
    public static Posn randomPosn(){
        return new Posn(randomInt(0,CDD.screenWidth),randomInt(0,CDD.screenHeight));
    }
    
     public static Posn randomAmbiPosn(){
        return new Posn(randomInt(Ambi.width/2,CDD.screenWidth-Ambi.width/2),randomInt(Ambi.height/2,CDD.screenHeight- Ambi.height/2));
    }
    ...
    public static Laser randomLaser(String type){
        switch(type){
            case "down" : return new Laser(new Posn(randomInt(0,CDD.screenWidth),0), 2, Laser.initSpeed);
        
            case "any" : return new Laser(new Posn(randomInt(0,CDD.screenWidth),randomInt(0,CDD.screenHeight)), randomInt(0,3), Laser.initSpeed);
        
            case "reset": return Laser.reset(Laser.initSpeed);
                
            default: return new Laser(new Posn(randomInt(0,CDD.screenWidth),randomInt(0,CDD.screenHeight)), randomInt(0,3), Laser.initSpeed);
        }
        }
   ...
   public static void testBoundsandMove(int trials,int paces){
        for(int i = 0; i<trials; i++){
            Ambi randomAmbiA = new Ambi(new Posn(randomInt(Ambi.width/2,CDD.screenWidth-Ambi.height/2),randomInt(Ambi.height/2,CDD.screenHeight-Ambi.height/2)), false);
            ...
            if(randomAmbiB.bufferOutOfBounds() != 
                    (randomAmbiB.center.x>CDD.screenWidth-Ambi.width/2||
                    randomAmbiB.center.x<Ambi.width/2||
                    randomAmbiB.center.y>CDD.screenHeight - Ambi.height/2||
   ...
   public static void testMoveLaser(int trials){
    ...
    public static void testLaserBounds(int trials){
        for(int i = 0; i<trials; i++){
            ...
            if(current.outOfBounds()!= next.outOfBounds()
                    && 
                    !(next.center.x > CDD.screenWidth + Laser.width
                    || next.center.x < 0 - Laser.width
                    || next.center.y > CDD.screenHeight + Laser.height
    ...
    public static void testWholeGame(int reps, int moves){
        ...
        //but unlikely
            ...
            CDD testWorld = new CDD(new Ambi(new Posn(CDD.screenWidth/2, CDD.screenHeight/2),"up", true),
        Laser.arrMoveCheckReset(new Laser[CDD.numberOfLasers], Laser.initSpeed), new DeceptiBot(randomAmbiPosn()), 0, Laser.initSpeed);
           ...
           //System.out.println(testWorld.laserSpeed);
            ...
            if(testWorld.dexter.center.x != CDD.screenWidth/2 
                    && testWorld.dexter.center.y != CDD.screenHeight/2){
                fishy1 = false;
            }
